...
%         One of the array entries must be at 25 degC.
% - cell_model: The output from proOCV, comprising the OCV model
...
function cell_model = processDyn(data,cell_model,numpoles,doHyst)
 global bestcost
 ...
 for k = ind25
     totDisgAh = data(k).script1.DisgAh(end) + ...
                 data(k).script2.DisgAh(end);
     totChgAh =  data(k).script1.ChgAh(end) + ...
                 data(k).script2.ChgAh(end);
     eta25 = totDisgAh/totChgAh;
     ...
     Q25 = data(k).script1.DisgAh(end) -...
           data(k).script1.ChgAh(end);
     data(k).Q = Q25; allQs(k) = Q25;
 end
 %dynamic_model.Q25 = Q25;
 
 for k = not25
     totDisgAh_n25 = data(k).script1.DisgAh(end)+ ...
                     data(k).script2.DisgAh(end);
     totChgAh_n25 =  data(k).script1.ChgAh(end)+ ...
                     data(k).script2.ChgAh(end);
     eta = totDisgAh_n25/totChgAh_n25;
     data(k).eta = eta; alletas(k) = eta;
     
     data(k).script1.ChgAh = eta*data(k).script1.ChgAh;
     Q = data(k).script1.DisgAh(end) - ...
         data(k).script1.ChgAh(end);
     data(k).Q = Q; allQs(k) = Q;
%dynamic_model.Q = Q;
 end
 
 cell_model.temps = unique(alltemps); numTemps = length(cell_model.temps);
 cell_model.etaParam = NaN(1,numTemps);
 cell_model.QParam = NaN(1,numTemps);
 
 for k = 1:numTemps
    cell_model.etaParam(k) = mean(alletas(alltemps == cell_model.temps(k)));
    cell_model.QParam(k)   = mean(allQs(alltemps == cell_model.temps(k)));
 end
 % -------------------------------------------------------------------
 % Step 2: Compute OCV for "discharge portion" of test
 % -------------------------------------------------------------------
 for k = 1:length(data)
    etaParam = cell_model.etaParam(k);
    etaik = data(k).script1.current; 
    etaik(etaik<0)= etaParam*etaik(etaik<0);
    data(k).Z = 1 - cumsum([0,etaik(1:end-1)])*0.1/(data(k).Q*3600); 
    data(k).OCV = OCVfromSOCtemp(data(k).Z(:),alltemps(k),cell_model);
 end
  % ------------------------------------------------------------------
  % Step 3: Now, optimize!
  % ------------------------------------------------------------------
  cell_model.GParam  = NaN(1,numTemps); % "gamma" hysteresis parameter
  cell_model.M0Param = NaN(1,numTemps); % "M0" hysteresis parameter
  cell_model.MParam  = NaN(1,numTemps); % "M" hysteresis parameter
  cell_model.R0Param = NaN(1,numTemps); % "R0" ohmic resistance parameter
  cell_model.RCParam = NaN(numTemps,numpoles); % time const.
  cell_model.RParam  = NaN(numTemps,numpoles); % Rk
  
  for theTemp = 1:numTemps 
    fprintf('Processing temperature %d\n',cell_model.temps(theTemp));
    bestcost = Inf;
%     theGammas = 1:5000;
...
%     figure(4); plot(theFit); stop
    if doHyst
      if exist('fminbnd.m','file')
        cell_model.GParam(theTemp) = abs(fminbnd(@(x) optfn(x,data,...
                                  cell_model,cell_model.temps(theTemp),...
                                  doHyst),1,250,options));
      else
        cell_model.GParam(theTemp) = abs(gss(@(x) optfn(x,data,...
                                  cell_model,cell_model.temps(theTemp),...
                                  doHyst),1,250,1e-8));
      end
    else
      cell_model.GParam(theTemp) = 0;
      %optfn(theGParam,data,cell_model,cell_model.temps(theTemp),doHyst);
      optfn(cell_model.GParam(theTemp),data,cell_model,cell_model.temps(theTemp),doHyst);
    end
    [~,cell_model] = minfn(data,cell_model,cell_model.temps(theTemp),doHyst);                          
  end
return
...
% --------------------------------------------------------------------
function cost = optfn(theGParam,data,cell_model,theTemp,doHyst)
 global bestcost
 
 cell_model.GParam(cell_model.temps == theTemp) = abs(theGParam);
  [cost,cell_model] = minfn(data,cell_model,theTemp,doHyst);
  
  if cost<bestcost % update plot of model params for every improvement
    bestcost = cost;
    disp('Best ESC model values yet!');
    figure(3); theXlim = [min(cell_model.temps) max(cell_model.temps)];
    subplot(2,4,1); plot(cell_model.temps,cell_model.QParam); 
                    title('Capacity (Ah)'); 
                    xlim(theXlim);
    subplot(2,4,2); plot(cell_model.temps,1000*cell_model.R0Param); 
                    title('Resistance (m\Omega)');
                    xlim(theXlim);
    subplot(2,4,3); plot(cell_model.temps,1000*cell_model.M0Param); 
                    title('Hyst Magnitude M0 (mV)');
                    xlim(theXlim);
    subplot(2,4,4); plot(cell_model.temps,1000*cell_model.MParam); 
                    title('Hyst Magnitude M (mV)');
                    xlim(theXlim);
    subplot(2,4,5); plot(cell_model.temps,getParamESC('RCParam',...
                    cell_model.temps,cell_model));
                    title('RC Time Constant (tau)');
                    xlim(theXlim);
    subplot(2,4,6); plot(cell_model.temps,1000*getParamESC('RParam',...
                    cell_model.temps,cell_model));
                    title('R in RC (m\Omega)');
                    xlim(theXlim);
    subplot(2,4,7); plot(cell_model.temps,abs(cell_model.GParam)); 
                    title('Gamma');    
                    xlim(theXlim);
  end
return
...
% --------------------------------------------------------------------
function [cost,cell_model]=minfn(data,cell_model,theTemp,doHyst)
    alltemps = [data(:).temp];
  ...
  G = abs(getParamESC('GParam',theTemp,cell_model));
  Q = abs(getParamESC('QParam',theTemp,cell_model));
  eta = abs(getParamESC('etaParam',theTemp,cell_model));
  RC = getParamESC('RCParam',theTemp,cell_model);
  numpoles = length(RC);
...
%       M=0; M0=0; R0 = W(1); Rfact = max([.001,.001,.001],W(2:end)');
    end
    ind = find(cell_model.temps == data(ind(thefile)).temp,1);
    cell_model.R0Param(ind) = R0;
    cell_model.M0Param(ind) = M0;
    cell_model.MParam(ind) = M;
    cell_model.RCParam(ind,:) = RC';
    cell_model.RParam(ind,:) = Rfact';
    ...
    v1 = OCVfromSOCtemp(0.95,data(ind(thefile)).temp,cell_model);
    v2 = OCVfromSOCtemp(0.05,data(ind(thefile)).temp,cell_model);
