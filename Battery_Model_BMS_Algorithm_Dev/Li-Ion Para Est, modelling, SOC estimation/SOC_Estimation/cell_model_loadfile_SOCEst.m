%% intializing the cell model simulation for SOC Estimation
clear;
load('OCV_DYN_FILES/SSG_DYN/SSG_DYN_P40.mat');  %data at 40 deg 
load('SSG_UDDS_2poles_model.mat');             %cell model parameters
numpoles=2;
R0=zeros(numpoles,1);
%dt=0.1;
T=40;
...
deltat = tk(2)-tk(1);
dt = deltat;
%out=sim('SOC_Estimation_EKF.slx');
%% Values Loading
...
% tk = (1:length(DYNData.script1.current))-1;
% %plot(tk,Z);
% 
soc     = cell_model.SOC;
...
% 
% ekfData = initEKF(voltage(1),T,SigmaX0,SigmaV,SigmaW,cell_model);
%% EKF initialization
etaParam = 1;
etaik = DYNData.script1.current; 
etaik(etaik<0)= etaParam*etaik(etaik<0);
...
%% model simulation
sim('SOC_Estimation_EKF')
%% Plot estimate of SOC
figure(1); clf; plot(tk/60,100*soc(1:end-1),'k',tk/60,100*sochat(1:end-1),'m'); hold on
plot([tk/60; NaN; tk/60],[100*(sochat(1:end-1)+socbound(1:end-1)); NaN; 100*(sochat(1:end-1)-socbound(1:end-1))],'m');
...
fprintf('RMS SOC estimation error = %g%%\n',sqrt(mean((100*(soc(1:end-1)-sochat(1:end-1))).^2)));

% Plot estimation error and bounds
figure(2); clf; plot(tk/60,100*(soc(1:end-1)-sochat(1:end-1)),'m'); hold on
h = plot([tk/60; NaN; tk/60],[100*socbound(1:end-1); NaN; -100*socbound(1:end-1)],'b');
...
ind = find(abs(soc(1:end-1)-sochat(1:end-1))>socbound(1:end-1));
fprintf('Percent of time error outside bounds = %g%%\n',...
        length(ind)/length(soc(1:end-1))*100);